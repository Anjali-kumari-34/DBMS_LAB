...
```sql
MariaDB [anjali_kumari]> DELETE FROM EMPLOYEE
...
```SQL
MariaDB [anjali_kumari]> SELECT E.ENAME,E.JOB,D.DNAME,D.LOCATION
    -> FROM EMPLOYEE E
    -> JOIN DEPARTMENT D
...
```SQL
MariaDB [anjali_kumari]> SELECT E.ENAME, E.SAL
    -> FROM EMPLOYEE E
    -> JOIN SALGRADE S
...
```SQL
MariaDB [anjali_kumari]> SELECT D.DNAME
    -> FROM DEPARTMENT D
...
```SQL
MariaDB [anjali_kumari]> SELECT M.ENAME
    -> FROM EMPLOYEE M
    -> WHERE M.JOB = 'MANAGER'
    -> AND M.SAL > (
    ->     SELECT AVG(E.SAL)
